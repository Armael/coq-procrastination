...
using \co{begin procrastination assuming f} (1). This gives us a new identifier
...
possible to call \co{procrastinate} at this point: the goal depends on the local
...
\co{procrastination assuming a b c}'', extended with the names \co{a},
\co{b} and \co{c}. Only side-conditions \co{P}, \co{Q} and \co{R} that make
sense in this context can be deferred using \co{procrastination}.

More generally, to each ``procrastination block'' (beginning with \co{begin
  procrastination} and ending with \co{end procrastination}) corresponds a proof
...
by \co{begin procrastination}. This assumption stores the current collection of
...
the enclosing block. When using \co{procrastinate} on a side-condition, the user
...
%% \co{procrastinate}, \emph{shelving} discharges a subgoal by putting it away and
...
its core, ``\co{begin procrastination} \co{assuming a}'' is nothing more than
``\co{eapply procrastinate\_1}'', where \co{procrastinate\_1} is the following
tautological lemma:
...
This is in fact precisely how the \co{procrastinate} tactic is implemented.
...
previous subsection). This is in essence how \co{begin procrastination} is
implemented.
